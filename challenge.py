...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # 简化的全局统计特征
        X_new["mean_all"] = np.mean(X, axis=1)
        X_new["missing_count"] = np.sum(pd.isnull(X), axis=1)
        ...
        if "Age" in X_new.columns:
            ...
            X_new["elderly_indicator"] = (X_new["Age"] > 65).astype(int)
            X_new["young_adult_indicator"] = ((X_new["Age"] >= 18) & (X_new["Age"] <= 40)).astype(int)

        # 温度特征 (只保留发热指标)
        if "max_Temp" in X_new.columns:
            X_new["fever_indicator"] = (X_new["max_Temp"] > 38.0).astype(int)
...
def main():
    ...
    # Use faster KNN imputation
    numeric_transformer = Pipeline(steps=[
        ("imputer", KNNImputer(n_neighbors=3)),  # 减少邻居数
        ("scaler", RobustScaler())
    ])
    ...
    param_grid = [
        {
            ...
            "clf__penalty": ["l1"],
            "clf__C": [ 0.1, 1, 10],
            "clf__class_weight": ["balanced", None, {-1: 1, 1: 1.2}, {-1: 1, 1: 1.5}, {-1: 1, 1: 2}, {-1: 1, 1: 5}],
        },
        {
            "poly__degree": [1],
            "clf__penalty": ["l2"],
